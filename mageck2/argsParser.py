...
def arg_count(subparser):
  ...
  cnt_pggroup.add_argument('--pairguide',choices=['none','firstpair','secondpair'],default='none',help='Search for second gRNA, located within the first pair or the second pair of the read. Specify the location of the guide with --pg-start/--pg-end (if --pairguide firstpair), or --pg-start-2/--pg-end-2 (if --pairguide secondpair); these are required. Note: the "auto" choice is disabled pending a reimplementation, as the search it used located UMIs rather than guides and never found a usable window.')
...
def arg_mle(subparser):
  ...
  bayesgroup.add_argument("-e","--negative-control",help="The gene name of negative controls. The corresponding sgRNA will be viewed independently. Part of the disabled --bayes module; currently an error -- for control-based normalization use --norm-method control with --control-sgrna.",default=None)
