...
from mageck2.mleclassdef import *
from mageck2.mledesignmat import *
from mageck2.mlemeanvar import *
# from mageck2.mlemultiprocessing import runem_multiproc
...
def em_whileloop(sk,beta_init_mat_0,size_vec,wfrac_list_0,alpha_dispersion,alpha_val,estimateeff,updateeff,removeoutliers,debug):
  ...
  beta1_se_mat=None
  beta1_new=None  
  # intermediate variables for calculating p values
  w_matrix=None
  xwx_inv=None
  sgrna_residule=None
  mu_estimate=None
  ...
  while(True):
    ...
    if True:
      ...
      # new method
      beta1_se_mat=beta1_new_pref*w_matrix*extdesign_mat*xwx_inv
  ...
  #
  #
  # use Chen-Hao's MAGeCK-NEST algorithm to calculate an updated version of beta_se_mat
  ...
  if True:
    ...
    v=extdesign_mat.shape[0]-(2*hat_matrix-hat_matrix*hat_matrix.getT()).trace()
    temp=((sgrna_residule/mu_estimate).getT()*(sgrna_residule/mu_estimate))/(v)
    beta1_se_mat=(temp.getA1()[0])*beta1_se_mat
...
def iteratenbem(sk,debug=True,estimateeff=False,updateeff=True,plot=False,alpha_val=0.01,meanvarmodel=None,restart=False,removeoutliers=False,size_factor=None,logem=True):
  ...
  if np.isnan(np.sum(beta_se_val)):
    logging.warning(sk.prefix+': se value equals to nan. Set to a large number.')
    beta_se_val=np.ones(len(beta_se_val))*1000000.0
  if np.sum(beta_se_val)==0:
    logging.warning(sk.prefix+': se value equals to zero. Set to a large number.')
    beta_se_val=np.ones(len(beta_se_val))*1000000.0
  #
  #beta_new_zscore=beta1_new.tolist()/beta_se_val
  ...
  sk.beta_pval_pos=sk.beta_pval
  sk.beta_pval_neg=sk.beta_pval
